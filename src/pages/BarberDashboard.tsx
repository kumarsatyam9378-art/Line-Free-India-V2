  ...
  return (
    <div className={`min-h-screen pb-40 animate-fadeIn bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] ${theme.bgGradient}`}>
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mt-2 mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            {!businessProfile?.isOpen ? (
              <div className="flex items-center gap-1.5 text-[10px] text-danger font-black uppercase tracking-widest bg-danger/10 border border-danger/30 px-2 py-1.5 rounded-md shadow-sm">
                <span className="w-1.5 h-1.5 rounded-full bg-danger" />Closed
              </div>
            ) : businessProfile?.isBreak ? (
              <div className="flex items-center gap-1.5 text-[10px] text-warning font-black uppercase tracking-widest bg-warning/10 border border-warning/30 px-2 py-1.5 rounded-md shadow-sm">
                <span className="w-1.5 h-1.5 rounded-full bg-warning animate-pulse" />
                Break {breakTimeRemaining > 0 ? formatBreakTime(breakTimeRemaining) : ''}
              </div>
            ) : (
            ...
            {/* Token Status Badge */}
            {businessProfile?.isOpen && businessProfile?.isStopped && (
              <div className="flex items-center gap-1.5 text-[10px] text-orange-500 font-black uppercase tracking-widest bg-orange-500/10 border border-orange-500/30 px-2 py-1.5 rounded-md shadow-sm">
                <span className="w-1.5 h-1.5 rounded-full bg-orange-500" />Tokens Paused
              </div>
            )}
          </div>
        </div>
        ...
        {/* Operations & Controls */}
        <div className="mb-6 p-1.5 bg-card rounded-3xl border border-border shadow-sm animate-slideUp overflow-hidden">
          {businessProfile?.isBreak ? (
            <button 
              onClick={handleEndBreak} 
              className="w-full py-3 bg-success/10 text-success font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-success/20 transition-colors active:scale-95 border border-success/20"
            >
              ✅ End Break {breakTimeRemaining > 0 ? `(${formatBreakTime(breakTimeRemaining)})` : 'Now'}
            </button>
          ) : (
            <div className="flex gap-1.5">
              <button onClick={() => handleStartBreak(15)} className="flex-1 py-3 bg-warning/10 text-warning font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-warning/20 transition-colors active:scale-95">
                ☕ 15m Break
              </button>
              <button onClick={() => handleStartBreak(30)} className="flex-1 py-3 bg-accent/10 text-accent font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-accent/20 transition-colors active:scale-95">
                🍝 30m Break
              </button>
              <button onClick={() => handleStartBreak(60)} className="flex-1 py-3 bg-danger/10 text-danger font-black text-[10px] uppercase tracking-widest rounded-2xl border border-danger/20 hover:bg-danger/20 transition-colors active:scale-95 shadow-inner">
                🍱 60m Break
              </button>
            </div>
          )}
                ...
                <div className="space-y-3">
                  {businessProfile.promoCodes.map((p, i) => (
                    <div key={i} className="flex justify-between items-center p-3 rounded-2xl border border-border bg-card-2 gap-2 shadow-sm">
                      ...
                      <button className="text-[10px] uppercase tracking-widest font-black text-text-dim px-3 py-2 neu-btn rounded-xl hover:text-text">
