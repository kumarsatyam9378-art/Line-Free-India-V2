...
export default function SmartNotifications() {
  ...
  return (<div className="min-h-screen bg-background pb-20 animate-fadeIn bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-900/20 via-background to-background">
    ...
    <div className="p-4 space-y-3">{notifications.length===0?<div className="text-center py-10 elite-glass border-dashed border-white/5 rounded-3xl opacity-50 spatial-card"><span className="text-3xl block mb-2">✅</span><p className="text-xs font-bold text-text-dim">All clear! No alerts.</p></div>:notifications.map((n,i)=>(<div key={i} className="flex items-start gap-3 p-4 rounded-2xl elite-glass spatial-card border-white/5"><span className="text-lg">{n.icon}</span><div className="flex-1"><p className={`text-xs font-black ${n.color}`}>{n.title}</p><p className="text-[10px] text-text-dim mt-0.5">{n.msg}</p></div><span className="text-[8px] text-text-dim font-bold whitespace-nowrap">{n.time}</span></div>))}</div></div>);
}
